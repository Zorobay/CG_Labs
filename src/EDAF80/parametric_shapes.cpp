...
parametric_shapes::createQuad(unsigned int width, unsigned int height) {
    ...
    unsigned int resolution = 50u;
    unsigned int vertices_nb = resolution*resolution;
    ...
    float height_step = static_cast<float>(height)/static_cast<float>(resolution);
    float width_step = static_cast<float>(width)/static_cast<float>(resolution);

    size_t index = 0;
    for(size_t row = 0; row < resolution; row++){
        for(size_t column = 0; column < resolution; column++){
            vertices[index] = glm::vec3(row*height_step, 0, column*width_step);

            tangents[index] = glm::vec3(1,0,0);
            binormals[index] = glm::vec3(0,0,1);
            normals[index] = glm::vec3(0,1,0);

            texcoords[index] = glm::vec3(static_cast<float>(row)*height_step/static_cast<float>(height),
                                         static_cast<float>(column)*width_step/static_cast<float>(width), 0);
            //std::cout << "(" << vertices[index].x << ", " << vertices[index].y << ", " << vertices[index].z << ")" << std::endl;
            index++;
        }
    }

    auto indices = std::vector<glm::uvec3>(3*(vertices_nb - 2));

    index = 0;
    for(size_t row = 0; row < resolution-1; row++){
        for(size_t column = 0; column < resolution-1; column++){
            indices[index++] = glm::uvec3(row*resolution + column, row*resolution + column + 1, (row+1)*resolution + column);
            indices[index++] = glm::uvec3(row*resolution + column + 1, (row+1)*resolution + column + 1, (row+1)*resolution + column);
        }
    }
